...
(async () => {
  ...
  httpServer.on("error", (err: any) => {
    if (err.code === "EADDRINUSE") {
      diployLogger.error(`Port ${port} is already in use. Please use a different port (set PORT in .env) or terminate the process using this port.`);
    } else {
      diployLogger.error(`Server error: ${err.message}`);
    }
    process.exit(1);
  });
  ...
  httpServer.listen(listenOptions, async () => {
    ...
    diployLogger.banner();
    diployLogger.success(`Server is UP and listening on ${actualHost}:${actualPort}`);
    diployLogger.info(`Local Access: http://localhost:${actualPort}`);
    
    // Find and display the local network IP address
    ...
    if (localIp) {
      diployLogger.info(`Local Network URL: http://${localIp}:${port}`);
    } else {
      diployLogger.info(`Local Network URL: http://0.0.0.0:${port}`);
    }
