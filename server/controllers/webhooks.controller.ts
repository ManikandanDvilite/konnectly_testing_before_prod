...
async function _sweepExpiredDedupRows(): Promise<void> {
  ...
  try {
    await db.execute(
      sql`DELETE FROM webhook_dedup WHERE created_at < NOW() - (${DEDUP_TTL_SEC} || ' seconds')::interval`
    );
  } catch {
...
async function _isAlreadyProcessedDb(wamid: string): Promise<boolean> {
  ...
  try {
    const result: any = await db.execute(sql`
      ...
      ON CONFLICT (wamid) DO UPDATE
        SET created_at = NOW()
        WHERE webhook_dedup.created_at < NOW() - (${DEDUP_TTL_SEC} || ' seconds')::interval
      RETURNING wamid
    `);
...
async function _isAlreadyProcessed(wamid: string): Promise<boolean> {
  // Try Redis first — works across multiple server instances and is fast.
  const acquired = await cacheSetNX(`wh:dedup:${wamid}`, "1", DEDUP_TTL_SEC);
  ...
  return _isAlreadyProcessedDb(wamid);
}
